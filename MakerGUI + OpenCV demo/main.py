...
class CameraWindow():
    def __init__(self, master):
        ...
        # ## Camera Pic ## #
        global tk_img   # IMPORTANT: PYTHON 的垃圾回收機制會把它回收，所以要用global
        tk_img = ImageTk.PhotoImage(pic_compress)

        # 在Tkinter介面上顯示影像
        self.label = Label(self.CameraWindow, image=tk_img)  # 在 Label 中放入圖片
        self.label.pack()
        ...
        # update picture -> 將影像匯入到tk_img
        self.camera = cv2.VideoCapture(0) # 選擇第二隻攝影機
        self.label.after(20, self.update)

    def update(self):
        # Get the latest frame and convert into Image
        cv2image = cv2.cvtColor(self.camera.read()[1], cv2.COLOR_BGR2RGB)
